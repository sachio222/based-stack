...
New skills not yet placed in `.skill-categories.yaml`. Move them to a category and rerun `sync-skills-index.py`.

| Skill | Description |
|---|---|
| `academic-cv-builder` | Format CVs for academic positions with publications, grants, and teaching |
| `app-launch-advisor` | Use when evaluating an app idea, generating app ideas, planning an app launch, figuring out what to do next with a mobile app, or assessing readiness to scale. Also use when the user mentions "app idea," "validate my app," "generate app ideas," "what app should I build," "how to market my app," "app funnel," "paywall strategy," "onboarding flow," or "organic content for my app." |
| `apply` | Fill out a job application on Greenhouse, Lever, or Workday |
| `cancer-board` | LP-aware oncology tumor board that reviews causal chain researcher output against the Lorenzato Process Methodology v2 (7 axioms, 11 patterns). Six specialists independently evaluate and provide actionable guidance — then peer-review each other and produce an Orchestrated Oncologist synthesis with verdict, LP axiom compliance, required actions, and recommended investigations. Examples: 'review the NSCLC analysis', 'run the cancer board', 'is this research ready for protocol use', or after any causal-chain-researcher run completes. |
| `career-changer-translator` | Translate skills from one industry to another, identify transferable skills |
| `coachs-guide-teaching` | Use when designing training sessions, giving athlete feedback, building coaching culture, checking for understanding, or developing athletes long-term. Covers feedback frameworks, session design, cognitive load, retrieval practice, and culture building. Use when the user mentions "coaching", "feedback", "session planning", "practice design", "checking for understanding", "cognitive load", "athlete development", "coaching culture", "teaching athletes", "drill design", or "long-term development". For team culture dynamics, see culture-code. For motivation systems, see drive-motivation. |
| `cover-letter` | Cover letter generation inside the proficiently job-search workflow. Reads the user's profile (`shared/templates/profile.md`) set up via the `setup` skill and pulls the job from the proficiently data store ('last' = most recently fetched). Choose THIS only when the user is in the proficiently pipeline (e.g., already ran `setup`, references the proficiently slash commands, or is invoking via `/proficiently:cover-letter`).  Pick `cover-letter-generator` instead when the user is not in the proficiently workflow and just wants a standalone cover letter from their resume + a job description. |
| `cover-letter-generator` | Standalone cover letter writer — takes a resume + job description and produces a tailored letter. Composable with the rest of the Paramchoudhary kit (`resume-tailor`, `job-description-analyzer`, etc.). Choose THIS for one-off cover letters when the user is NOT in the proficiently job-search workflow.  Pick `cover-letter` instead when the user is in the proficiently pipeline (already ran `setup`, references the proficiently slash commands, or says "the latest job"/"last job"). |
| `creative-portfolio-resume` | Balance visual design with ATS compatibility for creative roles |
| `ecological-dynamics` | Use when designing practice environments, manipulating constraints, building curricula around movement problems rather than techniques, or applying the constraints-led approach (CLA) to coaching. Covers affordances, perception-action coupling, representative learning design, self-organization, variability, and nonlinear pedagogy. Use when the user mentions "ecological dynamics", "constraints-led", "CLA", "affordances", "perception-action coupling", "representative practice", "nonlinear pedagogy", "self-organization", "variability", "task constraints", "game-based learning", "small-sided games", "degeneracy", "essential variables", or "differential learning". For coaching language and cueing, see language-of-coaching. For session design and feedback, see coachs-guide-teaching. For team culture, see culture-code. |
...
| `jobsearch-telegram` | Poll Telegram for job search messages — apply to jobs, search for roles, check status, all via chat |
| `language-of-coaching` | Use when designing coaching cues, giving movement feedback, building cueing systems, or optimizing how coaches communicate with athletes. Covers the cueing continuum (internal to external), analogy construction, the 3D cueing model, DDCDD communication loop, and attentional focus research. Use when the user mentions "coaching cues", "cueing", "external focus", "internal focus", "analogy", "movement instruction", "coaching language", "attentional focus", "verbal feedback", "motor learning", "how to cue", or "coaching communication". For session design and checking for understanding, see coachs-guide-teaching. For team culture dynamics, see culture-code. |
...
| `orchestrate-treatment` | Run the Lorenzato Process: full analysis (causal chain research + kinetic modeling) or kinetics-only on a previous run. Produces Research Brief, Clinical Brief, and Kinetic Analysis in one orchestrated command. |
...
| `resume-tailor` | Lightweight, single-resume tailoring — reorder sections, swap keywords, rewrite bullets to match a job description. Designed to chain with the rest of the Paramchoudhary kit (run `job-description-analyzer` first; pair with `resume-ats-optimizer`, `resume-bullet-writer`, `cover-letter-generator`). Choose THIS when the user has one existing resume and wants fast, composable customization.  Pick a different skill instead when: - The user wants live company research + a discovery interview to surface hidden experience, or has a `resumes/` library of multiple drafts → use `resume-tailoring`. - The user is in the proficiently job-search pipeline → use `tailor-resume`. |
| `resume-tailoring` | Deep, multi-step resume tailoring with live company/role research and a branching conversational interview to surface undocumented experience. Choose THIS over `resume-tailor` or `tailor-resume` when the user has a `resumes/` library of multiple drafts/versions OR wants the thorough, standalone treatment (research + interview + multi-format output). Does not depend on other skills.  Pick a different skill instead when: - The user has a single existing resume and wants quick keyword/bullet adjustments → use `resume-tailor`. - The user is in the proficiently job-search pipeline (`setup`, `job-search`, `apply`, `cover-letter`) → use `tailor-resume`. |
...
| `scientific-skills` | Scientific research assistant with 170+ skills for bioinformatics, cheminformatics, drug discovery, clinical research, machine learning, data visualization, statistics, genomics, proteomics, materials science, quantum computing, and more. Use when the user mentions anything scientific, asks to analyze data, create visualizations, query databases (PubMed, ChEMBL, UniProt, ClinVar, etc.), or work with scientific Python packages (RDKit, Scanpy, pandas, plotly, scikit-learn, PyTorch, etc.). |
| `setup` | One-time onboarding - upload resume, set preferences, and do a work history interview |
| `startup-os` | Autonomous startup operating loop with an all-star executive board. Maintains persistent operating context, runs board deliberation cycles, proposes actions for founder approval, executes via worktree-isolated agents, and documents everything. Sub-commands: init, status, approve, schedule, board. Use when managing a startup project, running a board review, or setting up recurring autonomous operations. |
...
| `tailor-resume` | Resume tailoring inside the proficiently job-search workflow (`setup` → `job-search` → `apply` / `cover-letter`). Reads the user's profile from `shared/templates/profile.md` set up by the `setup` skill. Choose THIS only when the user is operating in the proficiently pipeline (e.g., already ran `setup`, references the proficiently slash commands, or is invoking via `/proficiently:tailor-resume <url>`).  Pick a different skill instead when: - The user is not in the proficiently workflow and has a single resume → use `resume-tailor`. - The user wants deep tailoring with company research and discovery interview → use `resume-tailoring`. |
| `tech-resume-optimizer` | Optimize resumes for software engineering, PM, and technical roles |
| `track-docs` | Optional, hook-driven PM layer for any project — turns on doc-drift prevention by writing a TRACKING.md flag-file with the exact tree of docs to maintain. When on, a UserPromptSubmit hook injects tracked-doc context before each turn, and a Stop hook runs a Haiku gate then dispatches parallel subagents to update each tracked doc after work. When off (or absent), both hooks no-op silently. Invoke when the user says "track docs", "set up doc tracking", "turn on PM tracking", "stop doc drift", "keep docs synced", or similar. |
...
| `init-vault` | Scaffold a new Obsidian vault with the canonical layout — WORKSTATE, ARCHITECTURE, CHANGELOG, README, CLAUDE.md, per-folder READMEs — so the living-docs auto-update hook can maintain it. Invoke when user says "init a new vault", "create a vault", "scaffold a vault", "new Obsidian vault", or similar. |
| `init-porto-vault` | Use when the user wants to stand up a multi-agent team operating system — a CoS/PM/Engineer/Product agent org with a build-loop, generational handoffs, and governance docs — inside a project vault, new or existing. Triggers include "init porto vault", "set up the agent team here", "I want the multi-agent team operating system for this project", "spin up a CoS/PM/engineer/product team", "stand up CoS/PM/engineer/product agents". |
...
For marketing site work, use a two-pass approach:

**Pass 1: Load Knowledge Frameworks** that shape how you think about the problem -- StoryBrand for messaging structure, Cialdini for persuasion, Hormozi for offer design, etc.

**Pass 2: Run Task Checklists** that validate and execute against what was produced -- copywriting checklist, page CRO audit, form optimization, SEO audit, etc.

Load `product-marketing-context` at the start of a project to ground everything in the product's specific positioning, audience, and voice.

---

## Common Combos

| Task | Pass 1: Knowledge Frameworks | Pass 2: Task Checklists |
|---|---|---|
| **Homepage copy** | `storybrand-messaging` + `made-to-stick` + `influence-psychology` | `copywriting` + `page-cro` |
| **Pricing page** | `hundred-million-offers` + `cro-methodology` | `pricing-strategy` + `page-cro` |
| **Free trial page** | `influence-psychology` + `cro-methodology` | `form-cro` + `signup-flow-cro` |
| **Conversion audit** | `cro-methodology` + `ux-heuristics` | `page-cro` + `analytics-tracking` |
| **SEO blog articles** | `storybrand-messaging` + `made-to-stick` | `content-strategy` + `seo-audit` + `schema-markup` |
| **AI search optimization** | (none needed) | `ai-seo` + `schema-markup` |
| **Referral program** | `contagious` + `one-page-marketing` | `referral-program` |
| **Full marketing plan** | `one-page-marketing` + `storybrand-messaging` + `hundred-million-offers` | `site-architecture` + `content-strategy` |
| **Quiz/lead magnet** | `scorecard-marketing` | `lead-magnets` + `form-cro` |
| **Retention/churn** | `improve-retention` + `hooked-ux` + `drive-motivation` | `churn-prevention` + `email-sequence` |
| **Positioning** | `obviously-awesome` + `blue-ocean-strategy` | `competitor-alternatives` |
| **Email nurture** | `influence-psychology` | `email-sequence` |
| **Social media** | `contagious` + `made-to-stick` | `social-content` |
| **Full-site CRO audit** | `cro-methodology` + `made-to-stick` + `influence-psychology` | `page-cro` + `product-marketing-context` |
| **New page CRO audit** | `cro-methodology` + `made-to-stick` + `influence-psychology` | `page-cro` + `product-marketing-context` |
| **Business decision** | `decisive` | (use WRAP framework directly) |
| **New feature validation** | `testing-business-ideas` + `lean-startup` | `ab-test-setup` |
| **Sprint/project scoping** | `shape-up` | (use shaping + betting process) |

---

## Subagent Types

When Claude uses the Agent tool, it can spawn specialized subagents. Each type has different capabilities.

| Type | What It Does | When to Use | Can Edit Files? |
|---|---|---|---|
| `developer` | Implements code with tests. Full tool access. | Building features, writing code, fixing bugs. The workhorse. | Yes |
| `Explore` | Fast codebase exploration. Searches files, reads code, answers questions about the repo. | Finding files, understanding patterns, "how does X work?" | No |
| `Plan` | Designs implementation approaches. Identifies critical files, considers tradeoffs. | Planning before building. "How should we approach X?" | No |
| `researcher` | Explores codebases and domains. Explains landscape, options, best practices. Read-only. | Learning about unfamiliar territory before planning. | No |
| `architect` | Understands architecture, project conventions, quality design patterns. | Architecture decisions, evaluating design options. | Yes |
| `debugger` | Systematic evidence gathering for complex bugs. | Hard-to-find bugs where the cause isn't obvious. | Yes |
| `quality-reviewer` | Reviews code and plans for production risks, project conformance, structural quality. | Code review before merging. Catches issues a human reviewer would flag. | Yes |
| `technical-writer` | Creates documentation optimized for LLM consumption. | Writing docs that Claude will read in future sessions. | Yes |
| `content-developer` | Writes and edits content following project style guides and templates. | Content chunks: articles, docs, marketing copy. | Yes |

**Subagent features:**
- **Worktree isolation**: `isolation: "worktree"` gives a subagent its own copy of the repo on a separate branch. Useful for parallel work that might conflict.
- **Background execution**: `run_in_background: true` for work you don't need immediately. You get notified when it finishes.
- **Parallel launch**: Send multiple Agent tool calls in a single message to run subagents concurrently.
...
```
1. Explore agent -> understand the codebase
2. /planner -> design the approach
3. /deepthink -> analyze key decisions
4. /decision-critic -> stress-test decisions
5. developer subagents (parallel, worktrees) -> build components
6. developer subagent -> integrate
7. /incoherence -> consistency check
8. preview tools -> visual verification
9. /problem-analysis -> if anything breaks
```

### Debug a Problem
```
1. /problem-analysis -> systematic investigation
2. Explore agent -> find relevant code
3. debugger subagent -> fix
4. developer subagent -> implement fix
5. preview tools -> verify
```

### Write Content
```
1. /product-marketing-context -> load context, set voice
2. content-developer subagent -> write the content
3. /incoherence -> cross-reference consistency
```

### CRO Audit (Per Page or Full Site)
```
Pass 1 (Frameworks): Load evaluation lens
1. /cro-methodology -> audit framework, hypothesis design, prioritization
2. /made-to-stick -> SUCCESs checklist (Simple, Unexpected, Concrete, Credible, Emotional, Stories)
3. /influence-psychology -> Cialdini's 6 principles (reciprocity, commitment, social proof, authority, liking, scarcity)

Pass 2 (Execution): Audit each page
4. /product-marketing-context -> load personas, differentiators, customer language, objections
5. /page-cro -> run full CRO checklist against each page

Output: Per-page findings with specific recommendations, prioritized by impact.
Run this on any new marketing page before publishing.
```

### Make a Decision
```
1. researcher subagent -> gather context
2. /deepthink -> structured analysis
3. /decision-critic -> adversarial review
4. Save decision rationale to a doc
```
...
- **Skills invoke immediately.** Don't analyze first, then invoke. The skill orchestrates its own exploration.
- **Subagents start fresh.** Each Agent invocation gets no prior context. Give it a complete task description with all the files it needs to read.
- **Parallel when independent, sequential when dependent.** If two subagents don't need each other's output, launch them in parallel.
- **Worktrees for parallel code changes.** If two subagents would edit the same files, give them worktrees. Merge after.
- **Background for non-blocking work.** If you're waiting on research while building something else, run the research agent in the background.
- **Quality skills after building, not during.** Run `/incoherence`, `/simplify` after the work is done, not while writing.
